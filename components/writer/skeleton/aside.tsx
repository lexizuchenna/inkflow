  ...
  return (
    <div className="flex flex-col h-full animate-pulse px-4">
      ...
      <div className="flex-1 space-y-8">
        {[1, 2].map((group) => (
          <div key={group} className="space-y-3">
            <div className="h-2 w-20 bg-foreground/5 rounded mb-4" />
            {[1, 2, 3].map((item) => (
              <div
                key={item}
                className="h-10 w-full bg-foreground/5 rounded-2xl"
              />
            ))}
          </div>
        ))}
      ...
      <div className="mt-auto pt-6 border-t border-border">
        <div className="h-24 w-full bg-foreground/5 rounded-[2rem]" />
      </div>
    </div>
  );
}
